...
async function initDatabase() {
  ...
  try {
    ...
    // Ensure is_preorder column exists in products table (migration for existing DBs)
    await client.query(`
    ...
    if (parseInt(productCheck.rows[0].count) === 0) {
      ...
      const defaultProducts = [
        ...
        },
        {
          ...
          desc_th: 'เส้นก๋วยเตี๋ยวข้าวอย่างดี เหนียวนุ่ม ไม่ขาดง่าย เหมาะทำผัดไทย', desc_de: 'Hochwertige Reisnudeln, ideal für Pad Thai und้ ซุป.', desc_en: 'High-quality rice noodles, perfect for Pad Thai and noodle soups.'
        },
        {
    ...
    for (const p of rows) {
      const query = `
        INSERT INTO products (
          ...
          in_stock, stock, rating, reviews, desc_th, desc_de, desc_en, is_preorder
        ) VALUES (
      ...
      const values = [
        ...
        p.in_stock, p.stock, p.rating, p.reviews, p.desc_th, p.desc_de, p.desc_en, p.is_preorder
      ];
