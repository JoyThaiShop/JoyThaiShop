...
function handleBannerUpload(input) {
  ...
  reader.onload = function (e) {
    ...
    img.onload = function () {
      ...
      uploadedBannerImageBase64 = canvas.toDataURL('image/jpeg', 0.7);
...
async function addRecommendedBanner() {
  ...
  const alreadyAdded = recommendedBanners.find(b => String(b.productId) === String(productId) && !String(b.id).startsWith('def'));
  if (alreadyAdded) {
    showToast('❌ สินค้านี้ถูกเพิ่มเป็นสินค้าแนะนำแล้ว', 'error');
    return;
  }
  const newBanner = {
    id: 'b' + Date.now(),
  ...
  try {
    ...
    if (res.ok) {
      ...
      const preview = document.getElementById('bannerImagePreview');
      const placeholder = document.getElementById('bannerImagePlaceholder');
      if (preview && placeholder) { preview.style.display = 'none'; placeholder.style.display = 'block'; }
    } else {
...
function renderAdminBanners() {
  ...
  const mobileContainer = document.getElementById('adminBannerCardsMobile');
  if (!tbody) return;
  tbody.innerHTML = '';
  if (mobileContainer) mobileContainer.innerHTML = '';

  if (recommendedBanners.length === 0) {
    if (mobileContainer) mobileContainer.innerHTML = '<p style="color:var(--text3);text-align:center;padding:20px;">ยังไม่มีสินค้าแนะนำ</p>';
    return;
  }

  recommendedBanners.forEach(function(b) {
    const product = activeProducts.find(function(p) { return String(p.id) === String(b.productId); });
    const productName = product ? product.name.th : ('Product ID: ' + b.productId);
    const imgUrl = b.image || (product ? product.image : '') || '';
    ...
    let desktopActions = '';
    if (isDefault) {
      desktopActions = '<span style="font-size:12px;color:var(--text3);">ค่าเริ่มต้นระบบ</span>';
    } else {
      if (hasCustomImage) {
        desktopActions += '<button class="btn-edit" style="background:rgba(255,107,53,0.12);color:#ff6b35;border:1px solid rgba(255,107,53,0.25);margin-right:8px;" onclick="clearBannerImage(\'' + b.id + '\')">📷 ลบรูปแบนเนอร์</button>';
      }
      desktopActions += '<button class="btn-delete" onclick="deleteRecommendedBanner(\'' + b.id + '\')">🗑️ ลบ</button>';
    }
    ...
    tr.innerHTML = '<td>' + imgCell + '</td>' +
      ...
      '</td><td>' + desktopActions + '</td>';
    tbody.appendChild(tr);

    // ── Mobile Card ──
    if (mobileContainer) {
      const card = document.createElement('div');
      card.style.cssText = 'background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:14px;display:flex;flex-direction:column;gap:12px;width:100%;box-sizing:border-box;';

      const imgHtml = imgUrl
        ? '<img src="' + imgUrl + '" alt="" style="width:64px;height:64px;object-fit:cover;border-radius:8px;border:1px solid var(--border);flex-shrink:0;">'
        : '<div style="width:64px;height:64px;border-radius:8px;border:1px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:28px;flex-shrink:0;background:var(--surface);">' + emoji + '</div>';

      let mobileActions = '';
      if (isDefault) {
        mobileActions = '<span style="font-size:12px;color:var(--text3);">ค่าเริ่มต้นระบบ</span>';
      } else {
        let btns = '';
        if (hasCustomImage) {
          btns += '<button onclick="clearBannerImage(\'' + b.id + '\')" style="flex:1;padding:8px;background:rgba(255,107,53,0.12);color:#ff6b35;border:1px solid rgba(255,107,53,0.3);border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;">📷 ลบรูป</button>';
        }
        btns += '<button onclick="deleteRecommendedBanner(\'' + b.id + '\')" style="flex:1;padding:8px;background:#e74c3c;color:white;border:none;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer;">🗑️ ลบออก</button>';
        mobileActions = '<div style="display:flex;gap:8px;width:100%;">' + btns + '</div>';
      }
      ...
      card.innerHTML =
        '<div style="display:flex;gap:12px;align-items:flex-start;">' +
          ...
          '<div style="flex:1;min-width:0;">' +
            '<strong style="font-size:14px;color:var(--text);display:block;word-break:break-word;">' + productName + '</strong>' +
            (isDefault ? '<span style="display:inline-block;margin-top:4px;font-size:11px;background:#3b82f6;color:white;padding:2px 8px;border-radius:4px;">ค่าเริ่มต้น</span>' : '') +
          '</div>' +
        '</div>' +
        '<div style="border-top:1px solid var(--border);padding-top:10px;">' + mobileActions + '</div>';
