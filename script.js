...
function renderAdminBanners() {
  ...
  recommendedBanners.forEach(b => {
    ...
    if (mobileContainer) {
      const card = document.createElement('div');
      card.style.cssText = 'background:var(--surface); border:1px solid var(--border); border-radius:8px; padding:12px; display:flex; flex-direction:column; gap:10px; width:100%;';
      
      const deleteImageBtnMobile = hasCustomImage
        ? `<button class="btn-edit" style="background:rgba(255, 107, 53, 0.12); color:#ff6b35; border:1px solid rgba(255, 107, 53, 0.25); padding:6px 12px; border-radius:6px; font-size:12px; cursor:pointer;" onclick="clearBannerImage('${b.id}')">📷 ลบรูป</button>`
        : '';

      const actionBtnHtml = isDefault
        ? `<span style="font-size:12px; color:var(--text3);">ค่าเริ่มต้นระบบ</span>`
        : `<div style="display:flex; gap:8px; justify-content:flex-end; width:100%;">
            ${deleteImageBtnMobile}
            <button class="btn-delete" style="background:#e74c3c; color:white; border:none; border-radius:6px; padding:6px 12px; font-size:12px; cursor:pointer;" onclick="deleteRecommendedBanner('${b.id}')">🗑️ ลบ</button>
          </div>`;

      card.innerHTML = `
        <div style="display:flex; gap:12px; align-items:center;">
          <img src="${imgUrl}" alt="" style="width:70px; height:45px; object-fit:cover; border-radius:4px; border:1px solid var(--border); flex-shrink:0;">
          <div style="flex:1; display:flex; flex-direction:column; gap:4px; min-width:0;">
            <strong style="color:var(--text); font-size:14px; word-break:break-word;">${productName}</strong>
            ${isDefault ? '<span class="admin-badge" style="background:#3b82f6; color:white; border:none; margin:0; width:max-content; font-size:11px;">ค่าเริ่มต้น</span>' : ''}
          </div>
        </div>
        <div style="display:flex; justify-content:flex-end; border-top:1px solid var(--border); padding-top:8px; width:100%;">
          ${actionBtnHtml}
        </div>
      `;
      mobileContainer.appendChild(card);
